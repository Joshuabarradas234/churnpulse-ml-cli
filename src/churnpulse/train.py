...
def train_cmd(
    csv: Path = typer.Option(
        ...
        dir_okay=False,
        readable=True,
        help="Path to CSV dataset (must include a churn/label column).",
    ),
    ...
    reports_dir: str = typer.Option("reports", "--reports-dir", help="Where to save report.md."),
):
    metrics = train_model(
        csv_path=str(csv),
        target=target,
    ...
    typer.echo("Training complete ✅", color=True)
